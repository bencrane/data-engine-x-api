...
def _extract_company_intel_payload(output_payload: dict[str, Any] | None) -> dict[str, Any]:
    ...
    company_domain = output.get("domain") or output.get("target_company_domain")
    if not isinstance(company_domain, str) or not company_domain.strip():
        return {}

    company_name = output.get("company_name") or output.get("target_company_name")
...
def main() -> int:
    ...
    print(f"Starting company intel briefing backfill for {len(TARGET_SUBMISSION_IDS)} submissions")

    for submission_id in TARGET_SUBMISSION_IDS:
        ...
        for pipeline_run in pipeline_runs:
            ...
            if not step_results:
                total_skipped += 1
                print(f"[SKIP] submission_id={submission_id} run_id={run_id} no succeeded step_results")
                continue

            for step_result in step_results:
                total_processed += 1
                payload = _extract_company_intel_payload(step_result.get("output_payload"))
                if not payload:
                    total_skipped += 1
                    print(f"[SKIP] submission_id={submission_id} run_id={run_id} missing required output fields")
                    continue
                ...
                print(
                    ...
                    f"company_domain={payload['company_domain']} "
                    f"client={payload['client_company_name'] or '(unknown)'} "
                    f"id={result.get('id')}"
                )
